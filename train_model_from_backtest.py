...
3. Entrena modelo LightGBM con class balancing
...
MEJORADO - FASE 2:
    - Class balancing (balanceo de clases win/loss)
    - Más árboles y profundidad
    - Early stopping más agresivo
    - Mejor handling de clases desbalanceadas
"""
...
def main():
    ...
    # =======================================================================
    # MEJORADO: Calcular class weights para balanceo
    # =======================================================================
    n_samples = len(y_train)
    n_losses = (y_train == 0).sum()
    n_wins = (y_train == 1).sum()
    
    # Weight inversamente proporcional a frecuencia de clase
    weight_loss = n_samples / (2 * n_losses) if n_losses > 0 else 1.0
    weight_win = n_samples / (2 * n_wins) if n_wins > 0 else 1.0
    
    sample_weights = y_train.map({0: weight_loss, 1: weight_win})
    ...
    # =======================================================================
    # MEJORADO: Parámetros más robustos
    # =======================================================================
    logger.info(f"\nTraining LightGBM with improved parameters...")
    
    model = lgb.LGBMClassifier(
        n_estimators=500,           # Más árboles (antes: 200)
        max_depth=8,                # Más profundidad (antes: 6)
        learning_rate=0.03,         # Learning rate menor para mejor generalización
        min_child_samples=10,       # Menos restrictivo
        num_leaves=63,              # Más hojas para capturar patrones
        subsample=0.8,              # Bagging para evitar overfitting
        ...
        verbose=-1,
        force_col_wise=True,
        class_weight='balanced'     # IMPORTANTE: Balanceo automático
    )
    ...
    if test_acc < train_acc - 0.15:
        logger.warning(f"⚠️  Possible overfitting: train={train_acc:.2%} >> test={test_acc:.2%}")
    elif test_acc > train_acc:
        logger.info(f"✅ Good generalization: test={test_acc:.2%} >= train={train_acc:.2%}")
    
    logger.info(f"\nTest Set Classification Report:")
    print(classification_report(y_test, test_preds, target_names=['Loss', 'Win'], digits=3, zero_division=0))
    ...
    if unique_preds == 1:
        logger.warning("⚠️  WARNING: Model is predicting only ONE class!")
    ...
    for threshold in [0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80]:
        ...
        # Estimación simple de P&L
        # Asume: Win=$7 avg (mix de TP1/TP2/TP3), Loss=$3.3 avg
