...
FASE B - LIMPIEZA:
- Se agrega PositionWatcher al loop de threads
  para detectar cierres externos de posiciones

Watchers:
- PendingOrderWatcher: Monitorea órdenes pendientes
- ManagementApplier: Aplica BE/MOVE_SL/CLOSE
- PositionWatcher: Detecta cierres externos (NUEVO)
"""
import asyncio
import threading

from core.state import BotState
from core.monitoring import PendingOrderWatcher, ManagementApplier, PositionWatcher
...
async def run_watcher(state: BotState, poll_interval: float = 1.0) -> None:
    ...
    - PositionWatcher: Detecta posiciones cerradas externamente
    ...
    position_watcher = PositionWatcher(state, poll_interval)
    ...
    logger.info(
        ...
        threads=[
            pending_thread.name,
            management_thread.name,
            position_thread.name,
        ],
    )
    ...
    while True:
        ...
        if not position_thread.is_alive():
            logger.warning("PositionWatcher murió, reiniciando...")
            position_thread = threading.Thread(
                target=position_watcher.start,
                daemon=True,
                name="PositionWatcher",
